...
//shared class variables
	...
	static HungryWordMover[] Hungrywrdshft;
	...
	public static void createHungryWordMoverThreads() {
		//initialize shared array of current words with the words for this game
		for (int i=0;i<noWords-(noWords-1);i++) {
			HungryWords[i]=new HungryWord(dict.getNewWord(),gameWindow.getValidMiddleYpos(),xLimit);
		}
		//create threads to move them
		for (int i=0;i<noWords-(noWords-1);i++) {
			Hungrywrdshft[i] = new HungryWordMover(HungryWords[i],words,dict,score,startLatch,done,pause);
		}
		//word movers waiting on starting line
		for (int i=0;i<noWords-(noWords-1);i++) {
			Hungrywrdshft[i] .start();
		}
	}
...
public static void main(String[] args) {
		...
		HungryWords=new HungryWord[noWords-(noWords-1)];
		Hungrywrdshft=new HungryWordMover[noWords-(noWords-1)];
